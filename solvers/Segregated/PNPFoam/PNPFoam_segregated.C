...
//    simpleControl simple(mesh);
    ...
    while(ECsystem.phiRun())
    {
        ...
        while (runTime.loop())
        {
            ...
            currentIter = 0;
            cPlusResidual = 10;
            while(currentIter++ < nOuterIter and cPlusResidual > 1e-6)//for (int i=0; i<nOuterIter; i++)
            {
                #include "cPlusEqn.H"
            }
                //#include "psiEEqn.H"
            currentIter = 0;
            cMinusResidual = 10;
            while(currentIter++ < nOuterIter and cMinusResidual > 1e-6)//for (int i=0; i<nOuterIter; i++)
            {
                #include "cMinusEqn.H"
            }
